...
// Estimate 3D position of the board using detected markers
  ...
  // SWAN: duplicated here!!!
void imageCallback_NoCameraInfo(const sensor_msgs::ImageConstPtr &msg)
{
  ROS_INFO("hahaha");
  ...
  // SWAN: set as global variables
  // Mat cameraMatrix(3, 3, CV_32F);
  // cameraMatrix.at<float>(0, 0) = 1053.317761;
  // cameraMatrix.at<float>(0, 1) = 0.00000000e+00;
  // cameraMatrix.at<float>(0, 2) = 966.701528;

  // cameraMatrix.at<float>(1, 0) = 0.00000000e+00;
  // cameraMatrix.at<float>(1, 1) = 1053.098120;
  // cameraMatrix.at<float>(1, 2) = 607.956075;

  // cameraMatrix.at<float>(2, 0) = 0.00000000e+00;
  // cameraMatrix.at<float>(2, 1) = 0.00000000e+00;
  // cameraMatrix.at<float>(2, 2) = 1.00000000e+00;

  // Mat distCoeffs(1, 5, CV_32F);
  // distCoeffs.at<float>(0, 0) = -0.143641;
  // distCoeffs.at<float>(0, 1) = 0.059996;
  // distCoeffs.at<float>(0, 2) = -0.000430;
  // distCoeffs.at<float>(0, 3) = -0.000093;
...
int main(int argc, char **argv) {
  ...
  nh_.param<string>("image_topic", image_topic, "/blackflys/image_raw");
  nh_.param<string>("cinfo_topic", cinfo_topic, "/blackflys/camera_info");
  // nh_.param<string>("image_topic", image_topic, "/camera/image_raw");
  // nh_.param<string>("cinfo_topic", cinfo_topic, "/no_camera_info");

  if (DEBUG)
  {
    ROS_INFO("******************************************************");
    ROS_INFO("[SWAN] image_topic = %s", image_topic.c_str());
    ROS_INFO("[SWAN] cinfo_topic = %s", cinfo_topic.c_str());
    ROS_INFO("******************************************************");
  }

  // SWAN
  bool w_camera_info = cinfo_topic.compare("/no_camera_info") != 0;
  ros::Subscriber sub;
  if (w_camera_info)
  {
  ...
  {
    sub = nh_.subscribe(image_topic, 1, imageCallback_NoCameraInfo);
